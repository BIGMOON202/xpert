...
enum WaistLevel { high, mid, low }

extension WaistLevelExtension on WaistLevel {
  String get apiFlag {
    switch (this) {
      case WaistLevel.high:
        return 'high';
      case WaistLevel.mid:
        return 'mid';
      case WaistLevel.low:
  ...
  String imageName({Gender? gender}) {
    switch (this) {
      case WaistLevel.high:
        return gender == Gender.male ? 'high_waist.png' : 'high_waist_female.png';
      case WaistLevel.mid:
        return gender == Gender.male ? 'mid_waist.png' : 'mid_waist_female.png';
      case WaistLevel.low:
        return gender == Gender.male ? 'low_waist.png' : 'low_waist_female.png';
    }
  }
  ...
  String get title {
    switch (this) {
      case WaistLevel.high:
        return 'At the waist level';
      case WaistLevel.mid:
        return 'Slightly below the waist level';
      case WaistLevel.low:
        return 'Well below waist level';
    }
  }
...
class _WaistLevelPageState extends State<WaistLevelPage> {
  ...
  Widget build(BuildContext context) {
              ...
              child: Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      Radio(
                          value: level?.index ?? 0,
                          groupValue: this.selectedLevel?.index ?? -1,
                          onChanged: (newVal) {
                            setState(() {
                              this.selectedLevel = WaistLevel.values[newVal as int];
                            });
                          })
                    ],
                  ))));
    ...
    Widget imageOptionWidget({WaistLevel? level}) {
      final imageName = level?.imageName(gender: this.widget.gender) ?? '';
      return GestureDetector(
        onTap: () {
          setState(() {
            this.selectedLevel = level;
        ...
        child: Container(
          decoration: BoxDecoration(
              ...
              color:
                  this.selectedLevel == level ? Colors.white.withOpacity(0.1) : Colors.transparent),
          child: Padding(
    ...
    var questionText = SessionParameters().selectedUser == UserType.endWearer
        ? 'Where do you wear your uniform pants?'
        : 'Where does the end-wearer wear uniform pants?';
    var middleText = Text(
      questionText,
      style: TextStyle(
          color: SessionParameters().mainFontColor, fontSize: 18, fontWeight: FontWeight.w600),
